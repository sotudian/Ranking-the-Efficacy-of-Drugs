...
def TESTING_Func_LamdaMART(Validation_Unique_QID,Learned_Model_i,Unified_All_Y_QID_X):
...
def LamdaMart_5FCV_Func(fold,Etas,Max_depths,Num_Trees,Min_Child_Weight,Type_Data,Splitting_Type,Num_Drugs):

    # Load data
    CellDrug_Blood = pd.read_csv('/home/Data_Drug_Selection/BigData_CellDrug'+Type_Data+'.csv')
    CellDrug_Blood=CellDrug_Blood.drop(columns=['Index']) 
    
    CellGene_Blood = pd.read_csv('/home/Data_Drug_Selection/BigData_CellGene'+Type_Data+'.csv')
            ...
            for MD in Max_depths:
                for NT in Num_Trees:
                    ...
                    params = {
                         ...
                         'eval_metric':'ndcg',
                         'verbosity':0}
                    Learned_Model = xgb.train(params, dtrain, NT)
                    
                    All_Thetas[Counter] = Learned_Model
                    Counter += 1
                    All_parameters.append([Counter, MCW, eta,MD,NT])
    ...
    for pp in range(len(All_Thetas)):    
        Predicted_NDCG = TESTING_Func_LamdaMART(Validation_Unique_QID,All_Thetas[pp],Unified_All_Y_QID_X)
        if pp==0:
            arr = Predicted_NDCG
        else:
    ...
    Predicted_NDCG_t = TESTING_Func_LamdaMART(Test_Unique_QID,All_Thetas[Best_Itr],Unified_All_Y_QID_X)
    ...
    print('\n ******     Best Parameters     ******  \n')
    print('Best min_child_weight: ',All_parameters[Best_Itr][1],'\n')
    print('Best Etas: ',All_parameters[Best_Itr][2],'\n')
    print('Best Max_depths: ',All_parameters[Best_Itr][3],'\n')
...
Min_Child_Weight = [1,510,50,100] 



Type_Data = ''  #  '_Blood_normilized'   '_Immune_normilized'  '_Lung_normilized'  ''
...
for fold in range(1,6):
    ...
    print('**** FOLD', fold, '  -----------------------')
    print('***********************************************') 
    Predicted_NDCGatk_Valid,Predicted_NDCGatk_Test = LamdaMart_5FCV_Func(fold,Etas,Max_depths,Num_Trees,Min_Child_Weight,Type_Data,Splitting_Type,Num_Drugs)
    A = ['Fold'+str(fold)]
